...
//* 1.2.0		12/02/2025	-	added st_getdtfmt determine date/time format										*
//*																													*
//*******************************************************************************************************************/
...
#define		st_frecdt(s,l,t)				xymorg::StringThing::_frecdt(s, l, t)
#define		st_frecndt(s, t)				xymorg::StringThing::_frecdt(s, strlen(s), t)
...
//
//  Recognition string templates for Date/Time formats
// 
	constexpr auto TPLT_RFC2822 = "a, n a n n:n:n";
	constexpr auto FMT_RFC2822 = "%a, %d %b %Y %H:%M:%S";
	constexpr auto TPLT_TIMESTAMP = "n-n-n n:n:n";
	constexpr auto FMT_TIMESTAMP = "%Y-%m-%d %H:%M:%S";
	constexpr auto TPLT_ISO8601X = "n-n-nan:n:n+n";
	constexpr auto FMT_ISO8601X = "%Y-%m-%dT%H:%M:%S%z";
	constexpr auto TPLT_ISO8601XNTZ = "n-n-nan:n:n";
	constexpr auto FMT_ISO8601XNTZ = "%Y-%m-%dT%H:%M:%S";
	constexpr auto TPLT_USDT = "n/n/n n:n:n a";
	constexpr auto FMT_USDT = "%m/%d/%Y %I:%M:%S %p";
	constexpr auto TPLT_EURDT = "n/n/n n:n:n";
	constexpr auto FMT_EURDT = "%d/%m/%Y %H:%M:%S";
	constexpr auto TPLT_SHYPHEN = "n-a-n n:n";
	constexpr auto FMT_SHYPHEN = "%d-%b-%Y %H:%M";
	constexpr auto TPLT_ISO8601B = "nana";
	constexpr auto FMT_ISO8601B = "%Y%m%dT%H%M%SZ";
	constexpr auto TPLT_RFC3339O = "n-n-n n:n:n+n";
	constexpr auto FMT_RFC3339O = "%Y-%m-%d %H:%M:%S%z";
	constexpr auto TPLT_RFC3339UTC = "n-n-n n:n:na";
	constexpr auto FMT_RFC3339UTC = "%Y-%m-%d %H:%M:%SZ";
	constexpr auto TPLT_RFC2822NS = "a, n a n n:n";
	constexpr auto FMT_RFC2822NS = "%a, %d %b %Y %H:%M";
	constexpr auto TPLT_TIMESTAMPNS = "n-n-n n:n";
	constexpr auto FMT_TIMESTAMPNS = "%Y-%m-%d %H:%M";
	constexpr auto TPLT_USDTNS = "n/n/n n:n a";
	constexpr auto FMT_USDTNS = "%m/%d/%Y %I:%M %p";
	constexpr auto TPLT_EURDTNS = "n/n/n n:n";
	constexpr auto FMT_EURDTNS = "%d/%m/%Y %H:%M";
	...
	public:
		...
		//  4  -  char *				-  Const Pointer to the start of the section of the buffer to be injected
	...
	#define		RTEST_COND_NTOM				150																		//  n to m occurrences
		...
		//  _frecdt
		//
		//  This function will return the date and (or) time format string for the passed date/time value. 
		//
		//  Parameters
		//
		//	1  -  char*				-	Pointer to the string to be recognised
		//	2  -  size_t			-	Length of the input string
		//	3  -  char*				-	Pointer to the buffer to hold the format string
		//
		//  Returns
		//
		//		bool		-	true if a valid (recognised) date time, otherwise false
		//
		//  API Use Notes:
		//
		//		The format string buffer (3) MUST be at least 30 bytes in length.
		//		The input string must be a minimum of 8 characters.
		//

		static bool _frecdt(const char* In, size_t InLen, char* Fmt) {
			char*		pCopy = nullptr;													//  Copy of input string
			size_t		RStrLen = 0;														//  Length of recognition string
			...
			//  Create a copy of the input string to perform recognition on
			pCopy = (char*) malloc(InLen + 1);
			if (pCopy == nullptr) return false;
			memcpy(pCopy, In, InLen);
			pCopy[InLen] = '\0';
			...
			//  Convert the copy to a recognition string
			RStrLen = _frecan(pCopy, true, strlen(pCopy));

			//  Compare the recognition string to the templates
			//  1. RFC822
			if (strcmp(pCopy, TPLT_RFC2822) == 0) {
				strcpy_s(Fmt, 30, FMT_RFC2822);
				free(pCopy);
				return true;
			}
			//  2. Timestamp
			if (strcmp(pCopy, TPLT_TIMESTAMP) == 0) {
				strcpy_s(Fmt, 30, FMT_TIMESTAMP);
				free(pCopy);
				return true;
			}
			//  3. ISO 8601 Extended
			if (strcmp(pCopy, TPLT_ISO8601X) == 0) {
				strcpy_s(Fmt, 30, FMT_ISO8601X);
				free(pCopy);
				return true;
			}
			//  4. ISO 8601 Extended (no timezone)
			if (strcmp(pCopy, TPLT_ISO8601XNTZ) == 0) {
				strcpy_s(Fmt, 30, FMT_ISO8601XNTZ);
				free(pCopy);
				return true;
			}
			//  5. American standard date/time
			if (strcmp(pCopy, TPLT_USDT) == 0) {
				strcpy_s(Fmt, 30, FMT_USDT);
				free(pCopy);
				return true;
			}
			//  6. European standard date/time
			if (strcmp(pCopy, TPLT_EURDT) == 0) {
				strcpy_s(Fmt, 30, FMT_EURDT);
				free(pCopy);
				return true;
			}
			//  7. Short Hyphenated form
			if (strcmp(pCopy, TPLT_SHYPHEN) == 0) {
				strcpy_s(Fmt, 30, FMT_SHYPHEN);
				free(pCopy);
				return true;
			}
			//  8. ISO 8601 Basic
			if (strcmp(pCopy, TPLT_ISO8601B) == 0) {
				strcpy_s(Fmt, 30, FMT_ISO8601B);
				free(pCopy);
				return true;
			}
			//  9. RFC 3339 with offset
			if (strcmp(pCopy, TPLT_RFC3339O) == 0) {
				strcpy_s(Fmt, 30, FMT_RFC3339O);
				free(pCopy);
				return true;
			}
			// 10. RFC 3339 in UTC
			if (strcmp(pCopy, TPLT_RFC3339UTC) == 0) {
				strcpy_s(Fmt, 30, FMT_RFC3339UTC);
				free(pCopy);
				return true;
			}
			// 11. RFC 2822 no seconds
			if (strcmp(pCopy, TPLT_RFC2822NS) == 0) {
				strcpy_s(Fmt, 30, FMT_RFC2822NS);
				free(pCopy);
				return true;
			}
			// 12. Timestamp no seconds
			if (strcmp(pCopy, TPLT_TIMESTAMPNS) == 0) {
				strcpy_s(Fmt, 30, FMT_TIMESTAMPNS);
				free(pCopy);
				return true;
			}
			// 13. US standard format no seconds
			if (strcmp(pCopy, TPLT_USDTNS) == 0) {
				strcpy_s(Fmt, 30, FMT_USDTNS);
				free(pCopy);
				return true;
			}
			// 14. European standard format no seconds
			if (strcmp(pCopy, TPLT_EURDTNS) == 0) {
				strcpy_s(Fmt, 30, FMT_EURDTNS);
				free(pCopy);
				return true;
			}
