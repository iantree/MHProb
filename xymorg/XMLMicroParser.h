...
//* 1.1.0 -		12/02/2025	-	Added getAttributeDateTime()														*
//*							-	Added marshallAttributes()															*
//*																													*
//*******************************************************************************************************************/
...
#include	"MISC/PairColl.h"																	//  Pairs Collection
...
//
	...
	public:
		...
		public:
			...
			//  PARAMETERS:
			//
			//		char*				-		Const pointer to the node from which the attribute is to be extracted
			//		char*				-		Pointer to the name of the attribute
			//		size_t&				-		Reference to the variable to hold the length of the attribute value
			//
			//  RETURNS:
			...
			//  marshallAttributes
			//
			//  This function will return a pointer to a new Pairs collection containing the name and value of each attribute
			//	on the current node.
			//
			//  PARAMETERS:
			//
			//  RETURNS:
			//
			//		ParColl*				-		Pointer to the attribute values in a pairs collection
			//
			//  NOTES:
			//
			//  

			PairColl*	marshallAttributes() {
				const char*		pENode = nullptr;														//  Pointer to the end of the node
				const char*		pAttr = nullptr;														//  Scanning pointer
				const char*		pAName = nullptr;														//  Pointer to the attribute name
				size_t			NameLen = 0;															//  Length of the name
				const char*		pValue = nullptr;														//  Pointer to the value
				size_t			ValueLen = 0;															//  Length of the value
				char			EQVal = 0;																//  Ending quote value
				PairColl*		pAC = nullptr;															//  Pairs Attributes collection

				//  Safety
				if (isNull()) return nullptr;
				if (isClosing()) return nullptr;

				//  Locate the end of the node
				pENode = strchr(pPosition, '>');
				if (pENode == nullptr) return nullptr;

				//  Locate the end of the node name
				pAttr = pPosition;
				while (*pAttr != ' ' && pAttr < pENode) pAttr++;
				//  Node has no attributes
				if (pAttr == pENode) return nullptr;

				//  Create the Pair Collection to hold the name/value pairs
				pAC = new PairColl();

				//  Scan over the attributes capturing the name and the value
				while (pAttr < pENode) {
					//  Eliminate any whitespace before the attribute name
					while (*pAttr == ' ' && pAttr < pENode) pAttr++;
					if (pAttr < pENode) {
						pAName = pAttr;
						NameLen = 0;
						...
						//  Determine the length of the attribute name
						while (*pAttr != ' ' && *pAttr != '=' && pAttr < pENode) {
							NameLen++;
							pAttr++;
						}
						if (pAttr == pENode) return pAC;												//  Malformed node
						if (NameLen == 0) return pAC;													//  Malformed attribute

						//  Skip forward to the start of the value
						while ((*pAttr == ' ' || *pAttr == '=') && pAttr < pENode) pAttr++;
						if (pAttr == pENode) return pAC;												//  Malformed node

						//  Now positioned at the opening quote of quoted string
						if (*pAttr == SCHAR_SQUOTE || *pAttr == SCHAR_DQUOTE || *pAttr == SCHAR_PSQUOTE) {
							//  Save the quoted string token
							EQVal = *pAttr;
							if (EQVal == SCHAR_PSQUOTE) EQVal = SCHAR_PEQUOTE;
							pAttr++;
							...
							while (*pAttr != EQVal && pAttr < pENode) {
								ValueLen++;
								pAttr++;
							}
							if (pAttr == pENode) return pAC;											//  Malformed node
							...
							//  Save the name/value pair to the collection
							pAC->addVariable(pAName, NameLen, pValue, ValueLen);
						}
						else return pAC;																//  Malformed node
					}
				}

				//  Return the captured name/value pairs collection
				return pAC;
			}
			...
			//
			//  This function will return a TM structure containing the date and time from the named attribute 
			//  in the current node.
			//
			//  PARAMETERS:
			//
			...
			//  RETURNS:
			//
			//		tm					-		tm structure holding the date and time
			//
			//  NOTES:
			// 
			//	
			...
			struct tm getAttributeDateTime(const char* szAttribute) {
				struct tm		ADT = {};																	//  Derived Date & Time
				size_t			ASLen = 0;																	//  String attribute length
				const char*		pAttr = getAttribute(szAttribute, ASLen);									//  Pointer to attribute value
				char			szSDT[129] = {};															//  String buffer
				char			szDFMT[32] = {};															//  Date/Time format

				//  Safety
				(void)pAttr;
				if (ASLen == 0) return ADT;
				if (ASLen > 128) return ADT;
				ASLen = getAttributeString(szAttribute, szSDT, 128);										//  Get the Date Time as a string

				//  Recognise (or not) the format of the string
				if (st_frecndt(szSDT, szDFMT)) strptime(szSDT, szDFMT, &ADT);
		...
		bool   isValid() const { return XMLIsValid; }
